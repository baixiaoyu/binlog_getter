...
var Start_binlog_file string = ""
var End_binlog_file string = ""

// var Start_gtidset GTIDSet = nil
// var End_gtidset GTIDSet = nil
...
func (b *BinlogSyncer) parseEvent(s *BinlogStreamer, data []byte) (string, error) {
	//skip OK byte, 0x00
	data = data[1:]
	...
	case *RotateEvent:
		...
		log.Info("rotate to %s", b.nextPos)

		Out_file.Close()
		if End_binlog_file != "" {
			if b.nextPos.Name == End_binlog_file {
				b.wg.Done()
				b.Close()
			}
		}

		Out_file, err = os.Create(b.nextPos.Name)
		...
		binary.Write(buf, binary.LittleEndian, BINLOG_MAGIC)
		if _, err := Out_file.Write(buf.Bytes()); err != nil {
			fmt.Printf("error==%+v", err)
		}

		return b.nextPos.Name, nil
